// Creator social layer for oneIP.io — posts, follows, comments, and creator settings
// (display name, bio, Pro flag, canned auto-replies). Identity = Solana wallet address.
// NOTE (MVP): posting/commenting trusts the wallet field; production should verify a signature.
...
const F = (n) => join(DATA, n);

let posts = [];      // [{id, creator, text, image, ts}]
let comments = {};   // postId -> [{id, fan, text, ts, auto}]
let follows = {};    // creator -> [fan,...]
let creators = {};   // wallet -> {wallet, displayName, bio, pro, autoReplies:[{keyword,reply}]}
let likes = {};        // postId -> [fan,...]
let subscribers = {};  // creator -> [fan,...]   (distinct from follows)
let notifs = {};       // wallet -> [{id, type, text, ts, read}]
let loaded = false;
let timer = null;

const newId = () => randomBytes(8).toString("hex");
async function read(n, def) { try { return JSON.parse(await readFile(F(n), "utf8")); } catch { return def; } }

export async function load() {
  posts = await read("posts.json", []);
  comments = await read("comments.json", {});
  follows = await read("follows.json", {});
  creators = await read("creators.json", {});
  likes = await read("likes.json", {});
  subscribers = await read("subscribers.json", {});
  notifs = await read("notifs.json", {});
  loaded = true;
}
...
function persist() {
  clearTimeout(timer);
  timer = setTimeout(() => {
    writeFile(F("posts.json"), JSON.stringify(posts, null, 2)).catch(() => {});
    writeFile(F("comments.json"), JSON.stringify(comments, null, 2)).catch(() => {});
    writeFile(F("follows.json"), JSON.stringify(follows, null, 2)).catch(() => {});
    writeFile(F("creators.json"), JSON.stringify(creators, null, 2)).catch(() => {});
    writeFile(F("likes.json"), JSON.stringify(likes, null, 2)).catch(() => {});
    writeFile(F("subscribers.json"), JSON.stringify(subscribers, null, 2)).catch(() => {});
    writeFile(F("notifs.json"), JSON.stringify(notifs, null, 2)).catch(() => {});
  }, 200);
}

// ---- creators ----
// proTier: "" (free) | "ai" ($10/mo AI clone) | "publish" ($39/mo, includes AI clone + multi-platform).
...
/** Legacy boolean grant → maps to the basic $10 AI tier (publishing needs an explicit "publish" tier). */
export function setPro(wallet, pro) { return setTier(wallet, pro ? "ai" : ""); }
// Pro subscription tiers ($10 AI / $39 publish) were dropped — the AI clone + multi-platform
// publishing are free creator tools now (the platform earns on the commerce/token loop, not SaaS).
...
export async function addComment({ postId, fan, text }) {
  ...
  // Auto-reply as the creator: first the canned keyword rules ("一般的回复"); if none match
  // and the creator is Pro with the AI clone enabled, draft a reply with Claude (src/ai.js).
  let reply = matchAutoReply(post.creator, text);
  let viaAI = false;
  if (!reply) {
    const c = creators[post.creator];
    if (hasAi(c) && c.aiEnabled && ai.hasKey()) {
