...
async function handler(req, res) {
  ...
  try {
    ...
    // ---- machine-facing SEO surface (generated by scripts/generate-seo.mjs into web/) ----
    // Served at the site root so crawlers find them at /sitemap.xml, /robots.txt, /llms.txt.
    if (req.method === "GET" && (path === "/sitemap.xml" || path === "/robots.txt" || path === "/llms.txt")) {
      const seoTypes = { "/sitemap.xml": "application/xml; charset=utf-8", "/robots.txt": "text/plain; charset=utf-8", "/llms.txt": "text/plain; charset=utf-8" };
      try {
        const body = await readFile(join(ROOT, "web", path.slice(1)), "utf8");
        res.writeHead(200, { "content-type": seoTypes[path], "cache-control": "public, max-age=3600" });
        return res.end(body);
