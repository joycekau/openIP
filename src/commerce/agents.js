...
// sales in their territory, as consideration for onboarding + local support. Zero-dep, data/agents.json.
...
const FILE = join(__dir, "..", "..", "data", "agents.json");
...
export async function load() {
  try { agents = JSON.parse(await readFile(FILE, "utf8")); } catch { agents = {}; }
  loaded = true;
}
export async function ensureLoaded() { if (!loaded) await load(); }
function persist() {
  clearTimeout(timer);
  timer = setTimeout(() => writeFile(FILE, JSON.stringify(agents, null, 2)).catch(() => {}), 200);
}
