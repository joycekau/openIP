...
const FILE = join(__dir, "..", "..", "data", "buybacks.json");
...
export async function load() {
  try { byCoin = JSON.parse(await readFile(FILE, "utf8")); } catch { byCoin = {}; }
  loaded = true;
}
export async function ensureLoaded() { if (!loaded) await load(); }
function persist() {
  clearTimeout(timer);
  timer = setTimeout(() => writeFile(FILE, JSON.stringify(byCoin, null, 2)).catch(() => {}), 200);
}
...
export async function execute({ coin, orderId, netProfitCents }) {
  ...
  // (TODO: flush escrow on launch). Real Solana mints never start with "Kol".
