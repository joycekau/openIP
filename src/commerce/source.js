...
// A creator (KOL) curates products from a China dropshipping supply chain; fans buy them in
// FIAT (handled in payments.js). The supplier ships (the KOL holds NO inventory). This module
// owns only the catalog + placing the fulfillment order with the supplier.
//
// Phase 1 source = "manual": products are curated in data/products.json (admin/CSV/Studio).
// The SAME interface later gets a 1688-distribution / dropship-SaaS implementation — callers
// (orders.js, server routes) never change. Mock fulfillment when no real supplier is wired,
// exactly like publish.js / sync.js.
...
const FILE = join(__dir, "..", "..", "data", "products.json");
...
export async function load() {
  try { products = JSON.parse(await readFile(FILE, "utf8")); } catch { products = {}; }
  loaded = true;
}
export async function ensureLoaded() { if (!loaded) await load(); }
function persist() {
  clearTimeout(timer);
  timer = setTimeout(() => writeFile(FILE, JSON.stringify(products, null, 2)).catch(() => {}), 200);
}
...
export function addProduct({ creator, coin, title, image, description, priceCents, costCents, currency, sku, supplier, type, contentUrl }) {
  if (!creator) throw new Error("creator (wallet) required");
