...
const FILE = join(__dir, "..", "..", "data", "orders.json");
...
// oneIP.ai revenue split (user-locked 2026-06-29). Net profit = gross fiat - supplier cost - payment
// fee = 100%, split THREE ways: 75% creator / 20% floor / 5% platform pool. (Same split for every
// type: product/digital/tip/reaction — tips & reactions just have zero cost.)
...
export async function load() {
  try { orders = JSON.parse(await readFile(FILE, "utf8")); } catch { orders = {}; }
  loaded = true;
}
export async function ensureLoaded() { if (!loaded) await load(); }
function persist() {
  clearTimeout(timer);
  timer = setTimeout(() => writeFile(FILE, JSON.stringify(orders, null, 2)).catch(() => {}), 200);
}
