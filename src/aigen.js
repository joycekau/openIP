...
const SYSTEM = {
  post: (c) => `You write a short, punchy social post as ${c || "a creator"} on oneIP.ai, first person, platform-neutral (X/TikTok/Instagram), 1-3 sentences, at most a couple of tasteful hashtags. Output only the post text — no preamble.`,
  product: (c) => `You write e-commerce copy for ${c || "a creator"}'s product on oneIP.ai. Output a catchy title (max 8 words) on the first line, then a 2-3 sentence description. No preamble.`,
  idea: (c) => `You are a content strategist for ${c || "a creator"} on oneIP.ai. Suggest 3 concrete, postable content ideas as a short bulleted list. No preamble.`,
};
